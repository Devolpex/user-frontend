...
function ExploreProducts() {
  // const { products, updateProducts } = useProductClientContext();
  const [listProducts, setListProducts] = useState([]); // Added state
  useEffect(() => {
    fetchProducts();
  }, [listProducts]);
  ...
  return (
    <section className="px-8 pt-4 font-semibold my-16 flex flex-col gap-8 justify-center items-center">
      ...
      <Link
        to="/products-page"
        className="align-middle w-1/6 select-none bg-gray-800 hover:bg-slate-700 focus:bg-slate-900 text-white font-sans font-bold text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 px-6 rounded-lg shadow-gray-900/10 hover:shadow-gray-900/20 focus:opacity-[0.85] active:opacity-[0.85] active:shadow-none block  bg-blue-gray-900/10 text-blue-gray-900 shadow-none hover:scale-105 hover:shadow-none focus:scale-105 focus:shadow-none active:scale-100"
        type="button"
      >
        See More
      </Link>
